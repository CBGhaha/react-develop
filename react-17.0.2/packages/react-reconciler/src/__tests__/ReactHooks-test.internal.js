...
describe('ReactHooks', () => {
  ...
  it('double-invokes components with Hooks in Strict Mode', () => {
    ...
    expect(renderCount).toBe(false ? 2 : 1);
    renderCount = 0;
    renderer.update(
      <StrictMode>
        <NoHooks />
    ...
    if (!require('shared/ReactFeatureFlags').disableModulePatternComponents) {
      ...
      expect(renderCount).toBe(false ? 2 : 1); // Treated like a class
      renderCount = 0;
      renderer.update(
        <StrictMode>
          <Factory />
    ...
    expect(renderCount).toBe(false ? 2 : 1); // Has Hooks
    renderCount = 0;
    renderer.update(
      <StrictMode>
        <HasHooks />
  ...
  it('double-invokes useMemo in DEV StrictMode despite []', () => {
    ...
    expect(useMemoCount).toBe(false ? 2 : 1); // Has Hooks
  });
